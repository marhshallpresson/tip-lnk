...
### 6. Normies Integration (Ethereum Identity Mirroring)
TipStack supports mirroring Ethereum Normies onto Solana as creator identities.
- **Auto-Avatar**: If a creator has claimed their Normie on Solana, TipStack automatically sets the Normie image as their profile picture.
- **Cross-Chain XP**: Normies on Solana earn XP and level up based on TipStack activity:
    - **Sending a Tip**: +10 XP per $1 tipped.
    - **Receiving a Tip**: +5 XP per $1 received.
- **Level Formula**: `Level = floor(sqrt(XP / 100)) + 1`.
- **Mirroring Logic**: Verification happens via Dynamic Labs (ETH wallet link). Claiming a Normie mints a representative Solana cNFT (Metaplex Bubblegum) that stores the pixel data and metadata.
- **API Endpoint**: `POST /api/solana/normies/claim` (requires `userId`, `ethAddress`, `normieId`).
