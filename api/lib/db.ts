...
export async function getCreatorBalance(userId: string, transaction?: any) {
    ...
    try {
        ...
        const totalTipsUSD = tips.reduce((acc: number, tip: any) => {
        ...
        const totalWithdrawnUSD = payoutRows.reduce((acc: number, payout: any) => {
            try {
...
async function initSchemaInternal() {
  try {
    ...
    if (!(await db.schema.hasTable('user_normie_progression'))) {
      console.log('🛡️ initSchema: Creating user_normie_progression table...');
      await db.schema.createTable('user_normie_progression', (table) => {
        if (db.client.config.client === 'pg') {
          table.uuid('id').primary().defaultTo(db.raw('gen_random_uuid()'));
        } else {
          table.uuid('id').primary();
        ...
        table.string('userId').references('id').inTable('user').onDelete('CASCADE');
        table.integer('ethNormieId').notNullable();
        table.string('solanaMint').unique();
        table.integer('currentLevel').defaultTo(1);
        table.integer('experiencePoints').defaultTo(0);
        table.timestamps(true, true);
        table.unique(['userId', 'ethNormieId']);
      });
    }

    const tables = ['user', 'tips', 'indexer_state', 'roles', 'session', 'user_roles', 'email_verification_token', 'password_reset_token', 'payouts', 'fiat_payment_intents', 'fiat_webhook_events', 'oauth_clients', 'oauth_tokens', 'analytics_daily', 'user_normie_progression'];
...
/**
 * Professional Normie Leveling Utility
 * Updates XP and calculates levels for Solana Normie mirrors.
 */
export async function updateNormieXP(userId: string, xpDelta: number) {
  if (!userId) return;
  try {
    const progression = await db('user_normie_progression').where({ userId }).first();
    if (!progression) return;

    const newXP = (progression.experiencePoints || 0) + xpDelta;
    ...
    if (newLevel > progression.currentLevel) {
      console.log(`✨ Normie Level Up: User ${userId} reached level ${newLevel}`);
    }
  } catch (err) {
    console.error('🛡️ Normie XP Fault:', err);
  }
}
