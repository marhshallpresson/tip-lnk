...
function TipWidgetContent({ fixedRecipient = null, onSuccess, handleClose = () => {}, theme = 'dark', accent = '#00D265' }) {
  ...
  const handlePay = async () => {
    ...
    if (paymentMethod === 'crypto') {
        ...
        if (isMobile()) {
            console.log('📱 Mobile detected. Triggering native deep link flow...');
            const uri = getSolanaPayUri(resolvedAddress, amount, selectedToken?.mint || 'So11111111111111111111111111111111111111112');
            
            // Detect user preference or availability to route correctly
            if (window.solflare) {
                window.location.href = getSolflareDeepLink(window.location.href);
            } else {
                // Default to Phantom as industry leader for mobile deep linking
